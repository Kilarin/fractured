...
function gen_very_simple(parms)
	...
	minetest.log("rmg gen_very_simple-> realm minp="..luautils.pos_to_str(parms.realm_minp).." maxp="..luautils.pos_to_str(parms.realm_maxp)..
	...
	minetest.log("rmg gen_very_simple-> isectsize="..luautils.pos_to_str(isectsize).." minposxz x="..minposxz.x.." y="..minposxz.y)
...
--we loop through as z,y,x because that is way the voxel info is stored, so it is most efficent.
	...
	minetest.log("rmg very_simple-> END chunk="..luautils.pos_to_str(parms.isect_minp).." - "..luautils.pos_to_str(parms.isect_maxp).."  "..chugent.." ms") --tell people how long
end -- gen_very_simple
